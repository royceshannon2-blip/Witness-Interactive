...
class UIController {
  ...
  constructor(eventBus, timelineSelector, missionRegistry, consequenceSystem, resultsCard, uiContent, components = {}) {
    ...
    // Initialize knowledge checkpoint and path classifier
    this.knowledgeCheckpoint = new KnowledgeCheckpoint();
   ...
   * @param {object} data - Event data containing roleId, missionId, outcomeId, survived, deathChance, modifiers
   * @private
   */
  handleGameComplete(data) {
    ...
    // Store outcome ID and survival data from SceneStateMachine
    if (data && data.outcomeId) {
      this.currentOutcomeId = data.outcomeId;
    }
    
    if (data && data.survived !== undefined) {
      this.currentSurvival = {
  ...
  populateHistoricalRipple(screen) {
    ...
    const introText = getRippleIntro(this.currentRoleId, pathVariant);
  ...
  populateKnowledgeCheckpoint(screen) {
    ...
    // Select 3 path-aware questions using KnowledgeCheckpoint
    const selectedQuestions = this.knowledgeCheckpoint.selectQuestions(roleQuestions, pathVariant);
