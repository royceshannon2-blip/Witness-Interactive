...
class AnalyticsTracker {
  ...
  constructor(eventBus) {
    ...
    // Traction metrics for social proof
    this.tractionMetrics = {
      shares: 0,
      surveyClicks: 0,
      discordClicks: 0
    };
  ...
  setupEventListeners() {
    ...
    // Track traction metrics
    this.eventBus.on('share:clicked', (data) => {
      this.tractionMetrics.shares++;
      console.log('[TRACTION] Share button clicked', {
        total: this.tractionMetrics.shares,
        roleId: data?.roleId,
        pathVariant: data?.pathVariant
      });
      this.logAction('share_clicked', data);
    });
    
    this.eventBus.on('survey:clicked', (data) => {
      this.tractionMetrics.surveyClicks++;
      console.log('[TRACTION] Survey link clicked', {
        total: this.tractionMetrics.surveyClicks,
        source: data?.source
      });
      this.logAction('survey_clicked', data);
    });
    
    this.eventBus.on('discord:clicked', (data) => {
      this.tractionMetrics.discordClicks++;
      console.log('[TRACTION] Discord link clicked', {
        total: this.tractionMetrics.discordClicks,
        source: data?.source
      });
      this.logAction('discord_clicked', data);
    });
  }
  ...
  endSession() {
    ...
    console.log('[AnalyticsTracker] Traction metrics:', this.tractionMetrics);
  }

  /**
   * Get traction metrics for social proof
   * @returns {object} Traction metrics object
   */
  getTractionMetrics() {
    return { ...this.tractionMetrics };
  }
